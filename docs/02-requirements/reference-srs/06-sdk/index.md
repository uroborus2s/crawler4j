...
- **可独立发布**：SDK 作为独立 Python 包发布（语义化版本），外部模块项目可单独安装/升级。
- **可本地开发**：提供最小 CLI 能力与模块脚手架（脚手架、任务生成、工作流生成、配置 UI 生成）。
...
5. SDK CLI（实现：`crawler4j_sdk/cli/commands.py`）
   ...
   - v1.0 已实现命令（命令名与关键副作用 MUST 稳定）：`init-model/add/new/list/add-workflow/add-ui`
