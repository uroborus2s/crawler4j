...
运行时模块 ID。策略里的 `execution.module` 必须填这个值，而不是显示名、目录名别名或 zip 文件名。
...
2. `sdk_version_range` 用简单的 `>=1.0.3`
3. 每加一个工作流，都同步更新 `module.yaml`
4. 不要把复杂 UI 声明、实验性字段和历史遗留字段一起塞进去

这样能显著降低你在 DevLink、策略解析和 zip 安装时的排错成本。
