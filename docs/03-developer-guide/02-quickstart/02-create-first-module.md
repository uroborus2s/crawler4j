...
本页以下示例以 `crawler4j-sdk 1.2.0` 为准；如果你在维护 1.x 或 2.x 时期的旧模块，先完成工具接口升级，再继续走新脚手架流程。
...
3. 已把 `module.yaml.sdk_version_range` 改到 `>=1.2.0`
...
打开 `tasks/fetch_hotels.py`，你会看到脚手架生成的模板。将其修改为以下符合 **SDK 1.2.0** 规范的简化示例：
...
class FetchHotelsTask(TaskScript):
    ...
    async def execute(self, ctx):
        ...
        # 3. 保存到模块数据集 (SDK 1.2.0 标准用法)
        ctx.tools.call("db.replace_records", dataset="hotels", records=hotels)

        # 4. 记录当前抓取状态（如下次翻页游标）
        ctx.tools.call("db.set_state", key="last_sync_time", value="2026-03-31")
...
- **数据操作**：通过 `ctx.tools.call("db.*", ...)` 访问正式数据能力。
...
在任务运行模板中，至少要保证下面两个值正确：

- `execution.module = module.yaml.name`
- `execution.workflow = 你要运行的工作流名`

如果 `module.yaml.name` 是 `hotel_demo`，而你在运行模板里写了目录名、显示名或别名，调试和运行都会找不到正确目标。
