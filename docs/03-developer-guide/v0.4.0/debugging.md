...
uv run crawler4j ui-action list
uv run crawler4j page list
uv run crawler4j data list
uv run crawler4j candidate list
uv run crawler4j cleanup list
```
...
切到宿主环境执行 DevLink 命令。`host devlink` 加载的是模块源码目录，路径建议写绝对路径，避免宿主进程的工作目录和模块目录混在一起。
...
uv run crawler4j host devlink add /absolute/path/to/your/module
```
...
`created_at` / `updated_at` 是真实宿主时间字段；`create_at` / `update_at` 是常见误写，也会被阻断。不要为了通过校验改成拼写近似字段。来源系统时间戳要写成业务名，例如 `source_created_at`。

如果数据表是 `managed_dataset`，还不能把 `record_index`、`record_key`、`run_status`、`record_status` 声明为业务字段。这些是托管快照表的宿主系统字段，由 Core 在 `module_datasets` 中维护。

## 页面调试

页面问题按这条线查：

1. `uv run crawler4j page list` 是否能列出页面
2. `@page.schema` 顶层是否是 `Page`
3. 被 `@page` 装饰的 load handler 是否真实存在
4. `DataTable.data_source` 是否引用已声明数据表或 handler
5. Hosted UI 按钮或 CRUD handler 是否引用已扫描的 `@ui_action`
6. workflow/component 调用的浏览器动作是否引用已扫描的 `@page_action`
...
# Shell B: crawler4j 宿主源码仓或已安装宿主环境
cd /absolute/path/to/crawler4j-host
uv run crawler4j host debug config --module-root /absolute/path/to/your/module
```
