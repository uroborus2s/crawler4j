...
创建完成后进入模块根。交互式初始化默认会安装依赖；如果初始化时选择跳过安装，再执行 `uv sync`。
...
- `module.yaml` 只保留模块元信息、升级源、配置默认值等静态宿主配置
- 运行能力来自代码装饰器
...
`module init` 已生成一组默认示例：`interfaces/labor.py`、`objects/api_labor.py`、`workflows/main_workflow.py`、`data/accounts.py`、`data/account_overview.py`、`candidates/ready_accounts.py` 和 `tasks/example_action.py`。不要重复执行同名 create 命令；新增业务能力时使用新的名称。
...
如果模块要使用候选环境或参与批量环境清理，再按 [构建模块](./build-modules.md) 补 `candidate create` 和 `cleanup create`，并把候选函数改到真实业务表和 `env_id` 字段上。
...
目标文件：`data/hotels.py` 和 `data/hotel_overview.py`。
...
`check structure` 先确认固定文件和目录；`manifest lock` 会扫描装饰器、检查对象图和数据字段保留名并写入 lock；`check full` 要求 `.crawler4j/manifest.lock.json` 已存在且与源码一致。lock 通过后会生成：
...
uv run crawler4j host devlink add /absolute/path/to/your/workspace/hotel_demo
uv run crawler4j host debug config --module-root /absolute/path/to/your/workspace/hotel_demo
```
...
`package build` 当前会先跑 `check full` 再刷新 lock，因此发布前必须先保证 manifest lock 已生成且最新。
...
# Shell B: crawler4j 宿主源码仓或已安装宿主环境
cd /absolute/path/to/crawler4j-host
uv run crawler4j host install preview /absolute/path/to/your/workspace/hotel_demo/dist/hotel_demo-0.1.0.zip --skip-remote-check
uv run crawler4j host install apply /absolute/path/to/your/workspace/hotel_demo/dist/hotel_demo-0.1.0.zip --skip-remote-check
```

DevLink 验证的是源码目录；本地 ZIP 验证的是正式安装包。正式安装前，至少确认 `manifest lock`、`check full`、`check release`、`package verify` 和本地 ZIP install preview/apply 都通过。
