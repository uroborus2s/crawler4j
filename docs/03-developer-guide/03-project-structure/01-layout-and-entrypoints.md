...
├── module_runtime.py     # [标准] 运行时逻辑点（环境选择器、Hooks、手动注册）
...
`module_runtime.py` 现在是标准模块文件。模块级逻辑统一放在这里，支持的扩展点包括：

- **环境选择器:** 使用 `@env_selector(...)` 声明可供 ATM “选择环境”模式调用的回调函数
...
-> (选择环境模式) shim select_env -> module_runtime @env_selector callback
-> (创建环境模式) REM create_env
...
- “选择环境”模式不会再给模块传规则树；模块只会拿到宿主枚举出来的 `ready` 环境候选集，然后自行返回目标环境 ID
