...
- `crawler4j add [name]`：在当前模块项目里交互式创建任务脚本
- `crawler4j new <name>`：在当前模块项目里非交互式创建任务脚本
- `crawler4j list`：列出当前模块项目里的任务脚本
- `crawler4j add-workflow <name>`：创建工作流模板并更新 `module.yaml`
- `crawler4j add-ui`：创建或补齐 `config_schema.json` 与 `ui_extension`

### 3.3 预留命令
`validate/build/pack/doctor` 仍可作为后续阶段命令保留，但不应再假定存在“轻量脚本项目”。
