...
**关联 ID：** `REQ-001`, `REQ-002`, `REQ-003`, `REQ-004`, `REQ-005`, `REQ-006`, `REQ-007`, `REQ-008`, `REQ-009`, `REQ-010`, `BUG-001`, `CR-001`, `CR-002`, `CR-004`, `CR-008`, `CR-009`, `CR-016`
**最后更新：** 2026-06-19
...
- 批量导入的核心边界是“宿主读取来源，模块处理业务”。Excel/CSV 文件、剪贴板和手工输入都属于桌面宿主交互面，模块运行时代码不应拿到本地路径、文件句柄或二进制内容。
- Hosted UI 现有按钮只覆盖页面内 `reload/open_page/ui_action` 等基础动作，不能表达页面/表格 toolbar 中的导入按钮，也不能表达打开宿主导入弹窗后再提交 payload 的复合动作。
- `@ui_action` 适合快速校验并写入导入暂存表；workflow 适合长耗时导入和复用对象图的场景。workflow 仍不能声明 parameters，导入 payload 应由宿主写入运行态元数据，例如 `ctx.runtime["import_payload"]`。
- 宿主只负责通用解析、预览、容量限制和脱敏，不负责业务字段含义、业务去重、暂存表结构或最终业务表写入。模块应通过 `@data_table` 和 `ctx.db` 实现批次表、明细表和后续业务导入状态。
- 结果展示需要两层：首次导入返回批次汇总，后续 `import_data_records` 页面展示逐条状态和“从暂存表导入业务表”的结果。
- 安全风险集中在大文件、错误文件类型、字段解析失败、敏感字段明文日志和模块误读本地文件；这些必须作为宿主侧限制和测试重点。
...
| P1 | `REQ-010` / `TASK-030` 到 `TASK-034` Hosted UI 批量导入 | 影响账号、Cookie 和其它业务数据的录入效率、错误可见性与敏感数据安全 |
