...
| 2026-04-23 | 继续按正确的中轴线 inset 口径缩小桌面图标：在保留 `app_icon_master.png -> png/icns/ico` 统一导出的前提下，把 `ICON_SCALE` 从 `0.958984375` 下调到 `0.955078125`，将运行时图标四边 inset 从 `(77, 78, 77, 78)` 收到 `(79, 80, 79, 80)`，对应把导出图标再收小约 4px | Codex |
