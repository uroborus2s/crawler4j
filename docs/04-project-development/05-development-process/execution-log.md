...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-06-28 | 调整 Core `browser.drag natural` 体感时长与采样点数：新增默认 `natural_drag_down_up_duration_range=(0.9, 2.8)` 与 `natural_drag_sample_rate_range=(54.0, 66.0)`，按 `down_dwell + move samples dt` 控制 `down -> up` 窗口，并按约 `60Hz` 生成移动点；trace 新增 `pre_pause` 便于模块/测试自检，`precise` 仅补齐同名 trace 字段。验证：聚焦用例先复现旧实现约 `26Hz` 采样后通过，`test_browser_tools.py` 整文件 `18 passed`，目标 `ruff check` 与 `git diff --check` 通过 | Codex |
