...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-06-16 | 将根应用 / 运行时版本提升到 `0.4.15`，修复 VirtualBrowser 来源代理解析优先级：当外部 API 同时返回结构化 `proxy.host/port/protocol` 和 `proxy.url=http://127.0.0.1:本地端口` 时，来源代理同步优先采用真实结构化代理，避免把本地转发地址保存为绑定 IP；历史已错误保存的环境可再次点击“同步来源代理”覆盖修复。定向回归 `51 passed`，目标 `ruff check`、`uv lock --check`、`.factory/project.json` JSON 校验与 `git diff --check` 通过；正式客户端包、tag / release 与真机升级证据仍需后续补齐 | Codex |
