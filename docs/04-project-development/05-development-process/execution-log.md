...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-04-25 | 修正“从已有环境导入”的唯一性口径：REM 未同步列表、重复导入复用与状态库唯一索引均改为按来源环境名称 `provider_env_name` 判断，`provider_env_id` / `external_id` 不再决定唯一性，仅作为来源元数据透传。定向回归覆盖 import manager、导入任务服务、环境列表 UI 调用路径与状态库迁移，共 `19 passed` | Codex |
...
| 2026-04-24 | 完成全局环境页“从已有环境导入”链路：REM 环境表新增 `provider/provider_env_id/provider_env_name/...` 来源元数据，`VirtualBrowser` 现可拉取“来源有、本地无”的未同步环境并导入；环境列表页新增 `从已有环境导入` 配置面板，用户可选择 `环境来源 / 目标模块 / 模块工作流 / 未同步环境` 后执行 `导入并执行`；ATM 复用固定 `env_id` 执行链路，模块在 `ctx.env_id`、`ctx.page` 与 `ctx.runtime.creation_params` 中收到 provider metadata 与 `import_mode="existing_env"`。定向回归 `120 passed`，目标文件 `ruff check` 通过 | Codex |
