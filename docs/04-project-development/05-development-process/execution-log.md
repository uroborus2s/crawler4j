...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-06-28 | 调整 Core `browser.drag natural` 体感总时长：新增默认 `natural_drag_total_duration_range=(1.2, 2.8)`，按 `pre_pause + down_dwell + move samples dt + release_pause` 控制整体拖动窗口，并在 trace 中新增 `pre_pause` 便于模块/测试自检；`precise` 仅补齐同名 trace 字段。验证：`test_browser_tools.py` 整文件 `18 passed`，目标 `ruff check` 通过 | Codex |
