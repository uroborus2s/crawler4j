...
| 2026-04-23 | 将桌面图标可见 bbox 收口到 `855x855`：基于母版导出枚举确认 `scaled_size=966x966` 时 PNG 非透明 bbox 为 `855x855`，因此把 `ICON_SCALE` 固定为 `0.943359375`，并同步放宽透明安全区测试上限到 `85px` | Codex |
| 2026-04-23 | 完成桌面图标壳层占比最后一轮微调：将共享图标母版导出缩放系数从 `0.95` 下调到 `0.94575`，让导出 PNG 的实际可见 bbox 高度从 `858px` 收到 `857px`，作为本轮 macOS / Windows 共用图标尺寸收口值 | Codex |
...
| 2026-04-23 | 继续微调桌面图标壳层占比：将共享图标母版导出缩放系数从 `0.952` 下调到 `0.95`，让 `1024x1024` 母版导出后的主图高度再缩小约 `2px`，用于继续压低 macOS Dock 与 Windows 壳层中的视觉占比 | Codex |
