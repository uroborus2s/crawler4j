...
- 2026-05-11：Hosted UI 内联 `DataTable(query_handler)` 签名契约已收紧。`crawler4j-contracts` 根导出 `HostedDataTableQuery`、泛型 `HostedDataTableQueryResult[RowType]`、`HostedPageParams`、`HostedPageLoadResult` 等页面 handler 类型；SDK scanner 现在会拒绝未按固定参数名、同步函数、参数类型和返回类型实现的查询 handler，并接受模块页面自定义 `TypedDict` row 类型。`columns[].visible` 已确认默认可见、显式 `False` 隐藏展示但保留 row 字段。`packages/crawler4j-sdk/src/cli/templates.py`、`docs/03-developer-guide/v0.4.0/{ui-and-data-table,reference-core-capabilities,decorators-and-object-assembly}.md`、`docs/04-project-development/04-design/{module-config-runtime-data-contract,module-hosted-ui-framework}.md` 与 `.factory/memory/` 已同步。
