...
- Windows 客户端 `关于 -> 检查更新/升级` 会在后台线程执行 Velopack 检查、下载和应用更新；下载较大的 `.nupkg` 时按钮会暂时禁用并显示进度文案，主窗口不应再出现长时间“未响应”
...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-05-18 | 修复 Windows 客户端点击宿主升级时 UI 主线程被 Velopack 同步下载阻塞的问题；更新检查、下载和应用更新改为后台线程执行 | Codex |
