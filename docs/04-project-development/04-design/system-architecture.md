...
- REM 继续拥有环境生命周期；模块通过 `candidates/*.py` 中同步或异步的 `@env_candidates` 只读函数声明候选集合与可选 context。
...
| 2026-04-15 | 历史记录：曾补记 ATM hooks / `TaskSignal` / `WAITING_CONFIRMATION` 生命周期链；0.4.0 当前实现已在 2026-04-30 移除 hooks 运行链 | Codex |
