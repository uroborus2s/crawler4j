...
- `load_handler(context: TaskContext, page_id: str, params: HostedPageParams | None = None) -> HostedPageLoadResult`
- `query_handler(context: TaskContext, table_id: str, query: HostedDataTableQuery, params: HostedPageParams | None = None) -> HostedDataTableQueryResult[RowType]`

`query_handler` 是 SDK 强校验边界：函数必须是同步函数，参数名必须完整固定为 `context/table_id/query/params`，`params` 的默认值必须是 `None`，并且必须使用 `crawler4j-contracts` 导出的 `HostedDataTableQuery`、`HostedPageParams`、`HostedDataTableQueryResult` 类型。返回类型支持 `HostedDataTableQueryResult[TypedDictRow]` 泛型，以便模块在页面内声明 row 字段类型。
...
- `ctx.tools.call("browser.*", ...)` 等其它宿主能力
