...
| `custom_table` | `select`、`where_*`、`order_by`、`limit`、`offset`、已声明 `join`、`group_by`、`count/sum/avg/min/max`、`upsert/update_where/delete_where` | 未声明 join、跨模块表、未注册 SQL、模块自管事务 |
...
| 高频计算明细、计费明细 | `@data_table(storage_mode="custom_table")` + `ctx.db.from_` / `ctx.db.into(...).upsert/update_where/delete_where` | schema 驱动的受控实体表，按主键或条件更新 |
