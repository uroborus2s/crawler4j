...
`layout.columns` 必须是精确整数 `1`、`2` 或 `3`，`bool` 不作为整数接受；`layout.gap` 可省略，若提供则必须是精确非负整数。未声明 `layout` 时兼容旧行为，保持一列。renderer 按字段声明顺序逐行填充网格；可用屏幕宽度不足时自动降低有效列数。对话框宽高受 renderer 所在屏幕的可用区域约束，超出可展示几何的合法 gap 会收敛为屏幕内的有效 spacing；表单内容继续滚动，操作按钮不进入滚动区。
