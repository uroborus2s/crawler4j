...
> 文件名保留历史路径，正文已按 0.4.0 正式方案改为环境候选函数。固定资源池、资格卡片和同步快照方案不再是当前分支契约。

## 1. 目标

- Service Job 在候选环境暂时不可用时进入等待，而不是失败重试。
- 模块开发者只写一个同步或异步只读函数，不维护宿主资源池同步流程。
...
- `EnvCandidateResult(candidates=..., context=...)`

结构化 context 必须是 compact UTF-8 JSON、禁止 NaN/Infinity 且不超过 65536 字节。候选 surface 提供只读 `ctx.db` 与宿主管理的异步 `http.request`；不提供浏览器或环境写入工具。
...
3. 在只读 `ctx.db` + 仅 `http.request` 工具面的候选运行面执行 provider 一次，并正确 await 返回值。
4. 将返回值归一为 env id 列表和可选 context。
5. 按返回顺序过滤 `READY + BROWSER + 无租约` 环境。
6. 取得租约后不重跑 provider，只复核宿主指纹风险、claim 与 `env_binding_field` 业务绑定。
7. 若仍有效，把同一次 context 注入 workflow 并启动任务；若宿主复核失效或被抢占，释放租约并回到等待席位。

候选函数异常属于配置或业务代码错误，任务失败收口；候选为空属于正常容量不足，Service Job 进入等待。

## 5. 模块开发者职责

模块开发者只维护模块自己的业务数据和候选函数。
...
- 每次选择只执行一次 provider；租约后复核宿主指纹、claim 与业务绑定，避免竞争中失效。
- 将候选运行面限制为只读数据库与 `http.request`，不暴露浏览器、文件或环境写入工具。
...
- 0.4.0 环境选择只有一条模块开发路径：`candidates/*.py` + 同步或异步 `@env_candidates` 只读函数。
...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-08-08 | 支持异步候选、结构化 context 和候选 HTTP surface；租约后改为宿主安全复核，不重跑 provider | Codex |
