...
- 如果没有 `resource_pool`，运行模板必须指定 `env_id`；0.4.0 不再接受旧 `selector_name/env_selector` 路径。
...
不要求模块开发者直接操作底层 metadata API。V1 已落地的宿主 `ctx.tools` 能力为：

- `env.bind_resource_pool(env_id, pool_name, eligible=True, reason="", exclusive=True)`
- `env.mark_resource_pool_eligible(env_id, pool_name, reason="")`
- `env.mark_resource_pool_ineligible(env_id, pool_name, reason)`
- `env.remove_resource_pool(env_id, pool_name)`
- `env.replace_resource_pool_snapshot(pool_name, entries)`

补充口径：

- 模块可以在本模块内封装薄 helper，但运行时代码不得依赖 `crawler4j-sdk`
...
- `env.replace_resource_pool_snapshot(pool_name, entries)` 的 `entries` 必须是这个资源池的完整权威列表；未出现的环境卡片会被宿主删除
...
- 离线对账、宿主启动恢复或批量扫描这类没有环境上下文的逻辑，必须显式传 `env_id`，或者直接调用 `env.replace_resource_pool_snapshot(...)`
...
- 宿主直接从当前资源池内可分配候选中取一个环境
- 当前实现不承诺额外业务排序；需要业务排序时，应通过资源池资格卡片把不可用候选提前标成 `eligible=false`
- `selector_name/env_selector` 属于 0.3.x 维护线，当前 0.4.0 分支不兼容

在新的队列模式下：

- 只有带 `resource_pool` 的路径里，当前轮没有可分配候选时，才不应直接把任务判失败
- 正确语义是“保持等待，等待下一个合适工位”
- 不带 `resource_pool` 时必须指定 `env_id`，否则运行模板校验失败
