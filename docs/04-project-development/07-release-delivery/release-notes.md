...
- 当前工作区根应用已切到 `0.4.13` 源码版本线，用于承接已有环境导入 workflow 契约、`env.get_proxy` 当前代理读取、多环境导入批次元数据，以及环境管理列表绑定 IP 展示；SDK / Contracts 保持 `0.4.2`
...
| Full test / lint / smoke | 历史全量通过（2026-05-18 `992 passed`）；本轮 0.4.13 环境列表绑定 IP 展示通过 REM UI + 版本服务定向回归 `27 passed`、目标 `ruff check`、`uv lock --check`、`.factory/project.json` JSON 校验与 `git diff --check` |
