...
- 当前工作区根应用已切到 `0.4.18` 源码版本线，用于 VirtualBrowser 随机指纹创建期不再下发具体指纹字段，并在随机指纹模式下把 `chrome_version` 每次随机为 `139..145`；SDK / Contracts 保持 `0.4.2`
...
| 项目 | 结果 |
|---|---|
| 版本相关单测（`test_version_service.py`） | 通过（2026-06-22 随 0.4.18 版本提升回归覆盖） |
| Root wheel/sdist build | 历史通过（2026-05-26 `uv run build crawler4j` 产出 `crawler4j 0.4.3` wheel/sdist；当前 0.4.18 尚未刷新 root build） |
...
| Full test / lint / smoke | 历史全量通过（2026-05-18 `992 passed`）；0.4.17 任务禁用 / REM 固定运行模板安全门聚焦回归 `115 passed`；当前 0.4.18 VirtualBrowser 随机指纹与版本服务聚焦回归 `56 passed`，目标 `ruff check`、`uv lock --check`、`.factory/project.json` JSON 校验与 `git diff --check` 通过 |
| Docs markdown tree | 历史通过（`docs-stratego source validate --repo-path .`）；本轮未重跑 |

## 4. 当前不建议直接发布的原因

- `0.4.18` 对应的 Git tag、正式 GitHub release 与交付批次仍未完成
- `ctrip` 真实站点 E2E 与正式 release closeout 仍未完成
- Windows 真机签名、安装和自更新留证仍未完成

## 5. 下一版发布前必须满足

- 按 [版本治理规则](version-governance.md) 复验 `0.4.18` 仍是目标正式版本，且 README / 包描述 / release 文档不再混用旧口径
