...
- 当前工作区根应用已切到 `0.4.11` 源码版本线，用于承接 Hosted UI DataTable 自定义行按钮分发到同名 `@ui_action` 的客户端修复；SDK / Contracts 仍保持 `0.4.1`
...
| 项目 | 结果 |
|---|---|
| 版本相关单测（`test_version_service.py`） | 通过（2026-06-13 随 0.4.11 版本提升定向回归覆盖） |
| Root wheel/sdist build | 历史通过（2026-05-26 `uv run build crawler4j` 产出 `crawler4j 0.4.3` wheel/sdist；当前 0.4.11 尚未刷新 root build） |
...
| Full test / lint / smoke | 历史全量通过（2026-05-18 `992 passed`）；本轮 0.4.11 版本提升与 Hosted UI 自定义行按钮修复范围通过版本服务和 MMS Hosted UI 定向回归，目标 `ruff check`、`uv lock --check`、`.factory/project.json` JSON 校验与 `git diff --check` |
| Docs markdown tree | 历史通过（`docs-stratego source validate --repo-path .`）；本轮未重跑 |

## 4. 当前不建议直接发布的原因

- `0.4.11` 对应的 Git tag、正式 GitHub release 与交付批次仍未完成
...
- 按 [版本治理规则](version-governance.md) 复验 `0.4.11` 仍是目标正式版本，且 README / 包描述 / release 文档不再混用旧口径
