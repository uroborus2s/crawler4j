...
- 当前工作区根应用已切到 `0.4.15` 源码版本线，用于修复 VirtualBrowser 来源代理解析优先级，避免来源同步把本地转发 URL 保存为绑定 IP；SDK / Contracts 保持 `0.4.2`
...
| 项目 | 结果 |
|---|---|
| 版本相关单测（`test_version_service.py`） | 通过（2026-06-16 随 0.4.15 来源代理解析修复回归覆盖） |
| Root wheel/sdist build | 历史通过（2026-05-26 `uv run build crawler4j` 产出 `crawler4j 0.4.3` wheel/sdist；当前 0.4.15 尚未刷新 root build） |
...
- `0.4.15` 对应的 Git tag、正式 GitHub release 与交付批次仍未完成
...
- 按 [版本治理规则](version-governance.md) 复验 `0.4.15` 仍是目标正式版本，且 README / 包描述 / release 文档不再混用旧口径
