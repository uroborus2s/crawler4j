...
- 根应用 0.4.37 承接环境管理代理与指纹手动更新交互收口；SDK 0.4.4 / Contracts 0.4.3 保持不变
...
| 项目 | 结果 |
|---|---|
| 版本相关单测与打包配置 | 通过（2026-07-13 客户端 0.4.37 全量 `1198 passed`，`uv lock --check` 通过） |
| Root wheel/sdist build | 上一轮通过（2026-07-10 产出 `crawler4j 0.4.30` wheel/sdist；0.4.37 尚未构建） |
...
| Full test / lint / smoke | 客户端 0.4.37 全量 `1198 passed`；全仓 Ruff、`uv lock --check`、`git diff --check` 与 UI smoke 通过 |
| Docs markdown tree | 通过（`docs-stratego source validate --repo-path .`，`pages=86 contracts=0`） |

## 4. 当前不建议直接发布的原因

- `0.4.37` 对应的桌面安装包、Git tag、正式 GitHub release 与交付批次仍待后续完成
- `ctrip` 真实站点 E2E 与正式 release closeout 仍未完成
- Windows 真机签名、安装和自更新留证仍未完成

## 5. 下一版发布前必须满足

- 按 [版本治理规则](version-governance.md) 复验 `0.4.37` 仍是目标正式版本，且 README / 包描述 / release 文档不再混用旧口径
