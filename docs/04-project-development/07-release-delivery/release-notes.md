...
| Root wheel/sdist build | 通过（2026-05-01 产物：`crawler4j-0.4.0` wheel/sdist） |
| SDK wheel/sdist build | 通过（2026-05-01 产物：`crawler4j-sdk 0.4.0` wheel/sdist） |
| SDK publish | 待按 `crawler4j-sdk 0.4.0` 执行正式发布流程 |
| Contracts wheel/sdist build | 通过（2026-05-01 产物：`crawler4j-contracts 0.4.0` wheel/sdist） |
| Contracts publish | 待按 `crawler4j-contracts 0.4.0` 重新执行正式发布流程 |
| Desktop PyInstaller bundle | 通过（2026-05-01 `uv run package-desktop` 产出 `packages/crawler4j/dist/desktop/macos/Crawler4j.app`） |
| Full test / lint / smoke | 通过（2026-05-01 `886 passed`，`ruff check .`、`git diff --check`、SDK CLI help、UI smoke 通过） |
| Docs markdown tree | 历史通过（`docs-stratego source validate --repo-path .`）；本轮未重跑 |

## 4. 当前不建议直接发布的原因

- `0.4.0` 对应的 Git tag、正式 release notes 与交付批次仍未完成
- `ctrip` 真实站点 E2E 与正式 release closeout 仍未完成
- `crawler4j-sdk 0.4.0`、`crawler4j-contracts 0.4.0` 尚缺 publish 证据
- Windows 真机签名、安装和自更新留证仍未完成

## 5. 下一版发布前必须满足

- 按 [版本治理规则](version-governance.md) 复验 `0.4.0` 仍是目标正式版本，且 README / 包描述 / release 文档不再混用旧口径
