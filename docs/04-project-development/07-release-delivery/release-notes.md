...
- 当前工作区根应用已切到 `0.4.14` 源码版本线，用于承接已导入指纹浏览器环境的来源代理同步、IP 表唯一匹配绑定和环境管理页批量同步入口；SDK / Contracts 保持 `0.4.2`
...
| 项目 | 结果 |
|---|---|
| 版本相关单测（`test_version_service.py`） | 通过（2026-06-16 随 0.4.14 来源代理同步回归覆盖） |
...
| Full test / lint / smoke | 历史全量通过（2026-05-18 `992 passed`）；本轮 0.4.14 来源代理同步、环境列表和版本服务定向回归 `49 passed`，目标 `ruff check`、`uv lock --check`、`.factory/project.json` JSON 校验与 `git diff --check` 通过 |
