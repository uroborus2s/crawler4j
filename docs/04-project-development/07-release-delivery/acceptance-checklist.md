...
| 类别 | 检查项 | 证据 | 当前基线状态 |
|---|---|---|---|
| 版本 | `packages/crawler4j/pyproject.toml` 与发布目标版本一致，运行时版本服务可正确读取 | `version-governance.md` | 已具备（当前根应用源码线 `0.4.39`，聚焦版本/打包回归 `165 passed`；最近正式 tag 仍为 `v0.2.0`） |
| 版本 | 当前工作区版本、最近正式 tag、SDK/Contracts 版本口径清楚 | `release-notes.md` | 已具备 |
| 测试 | `uv run pytest -q` 通过 | `test-plan.md` | 部分具备（2026-07-15 全量 `1235 passed`，另有 13 项既有沙箱/只读数据库环境基线；CR-022 定向与版本/打包回归全部通过） |
| 测试 | `uv run ruff check .` 通过 | `test-plan.md` | 已具备（2026-07-15 全仓复验通过） |
| 运行 | `uv run python scripts/smoke_test_ui.py` 通过 | `test-plan.md` | 已具备（2026-07-15 复验通过；覆盖 Shell 结构与 Dashboard 异步刷新） |
| 构建 | Root / SDK / Contracts build 通过 | `test-plan.md` | 已具备（root 0.4.39、SDK 0.4.5、Contracts 0.4.4 wheel/sdist 均已构建；桌面安装包仍为独立 gate） |
