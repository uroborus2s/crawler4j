...
| 类别 | 检查项 | 证据 | 当前基线状态 |
|---|---|---|---|
| 版本 | `packages/crawler4j/pyproject.toml` 与发布目标版本一致，运行时版本服务可正确读取 | `version-governance.md` | 已具备（当前根应用源码线为 `0.4.6`，最近正式 tag 仍为 `v0.2.0`，本轮版本服务 + 指纹浏览器生命周期定向回归 `56 passed`） |
...
| 构建 | Root / SDK / Contracts build 通过 | `test-plan.md` | 部分具备（2026-05-18 已产出 SDK / Contracts 0.4.1；2026-05-26 已产出 Root 0.4.3 wheel/sdist，当前 0.4.6 root build 尚未刷新） |
...
2. 当前 `0.4.6` 对应的 Git tag / GitHub release 资产与本次交付批次未在本轮复核闭环；SDK / Contracts PyPI publish 与 macOS 0.4.3 客户端升级包已完成。
