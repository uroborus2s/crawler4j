...
| 类别 | 检查项 | 证据 | 当前基线状态 |
|---|---|---|---|
| 版本 | `packages/crawler4j/pyproject.toml` 与发布目标版本一致，运行时版本服务可正确读取 | `version-governance.md` | 已具备（当前根应用源码线 `0.4.42`；最近正式 tag 仍为 `v0.2.0`） |
| 版本 | 当前工作区版本、最近正式 tag、SDK/Contracts 版本口径清楚 | `release-notes.md` | 已具备 |
| 测试 | `uv run pytest -q` 通过 | `test-plan.md` | 已具备（2026-08-09 全量 unit `1288 passed`） |
| 测试 | `uv run ruff check .` 通过 | `test-plan.md` | 已具备（2026-08-09 全仓复验通过） |
| 运行 | `uv run python scripts/smoke_test_ui.py` 通过 | `test-plan.md` | 已具备（2026-07-15 复验通过；覆盖 Shell 结构与 Dashboard 异步刷新） |
| 构建 | Root / SDK / Contracts build 通过 | `test-plan.md` | 已具备（root 0.4.42、SDK 0.4.6、Contracts 0.4.5 wheel/sdist 均已构建；SDK/Contracts 已发布并通过隔离安装；root 0.4.40 wheel HTTP2 隔离 smoke 为历史证据） |
...
2. 当前 `0.4.42` 尚未构建桌面发布资产；`0.4.40` 已构建本机 macOS PyInstaller app 并通过 HTTP 运行时诊断，但签名 DMG/更新源、Windows 包、Git tag / GitHub release 资产仍待后续创建；SDK 0.4.6 / Contracts 0.4.5 已完成 PyPI 发布。
...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-08-09 | 根应用源码提升到 `0.4.42`；保活任务在并发已满时不再探测候选函数，全量 unit `1288 passed`、Ruff、lock、root build 与文档校验通过，桌面发布资产仍未触发 | Codex |
| 2026-08-08 | 根应用源码提升到 `0.4.41`，Contracts / SDK 源码提升到 `0.4.5` / `0.4.6`；源码与 wheel 验证纳入 CR-025，桌面发布资产仍未触发 | Codex |
| 2026-07-19 | 根应用提升到 `0.4.40`；`API-024`、wheel 隔离安装和 macOS PyInstaller HTTP 运行时通过，Windows 与外部 ctrip 模块接线仍为发布 gate | Codex |
| 2026-07-15 | 根应用提升到 `0.4.39`、SDK 提升到 `0.4.5`、Contracts 提升到 `0.4.4`；聚焦 `175 passed`、全量 `1235 passed` 加 13 项已知环境基线，全仓 Ruff、lock、UI smoke、三包 build 与 dry-run 通过 | Codex |
| 2026-07-14 | 根应用版本提升到 `0.4.38`，用于发布 `env.cookie.ensure` 分阶段脱敏错误诊断；全量 `1201 passed`、全仓 Ruff、`uv lock --check`、Root build 与 diff 检查通过，SDK / Contracts 不升级 | Codex |
| 2026-07-13 | 根应用版本提升到 `0.4.37`，用于环境管理代理与指纹手动更新交互收口；全量 `1198 passed`、全仓 Ruff、`uv lock --check`、UI smoke 与 diff 检查通过，桌面安装包与 Windows 真机测试仍待补齐 | Codex |
| 2026-07-12 | 根应用版本提升到 `0.4.36`，用于 DevLink 模块并发强制重载序列化；全量 `1190 passed`、全仓 Ruff、`uv lock --check`、UI smoke 与 diff 检查通过，桌面安装包与 Windows 真机测试仍待补齐 | Codex |
| 2026-07-11 | 根应用版本提升到 `0.4.35`，用于 VirtualBrowser 随机后最小指纹修正与创建验收；全量 `1189 passed`、全仓 Ruff、`uv lock --check`、UI smoke 与 diff 检查通过，桌面安装包与 Windows 真机测试仍待补齐 | Codex |
| 2026-07-11 | 根应用版本提升到 `0.4.34`，用于 VirtualBrowser 启动后完整代理状态回写；全量 `1182 passed`、全仓 Ruff、`uv lock --check`、UI smoke 与 diff 检查通过，桌面安装包与正式发布 Gate 待补齐 | Codex |
| 2026-07-11 | 根应用版本提升到 `0.4.33`，已完成全量 `1182 passed`、全仓 Ruff、`uv lock --check` 与 diff 检查；正式发布 Gate 仍受桌面安装包、真实站点 E2E 和 Windows 真机证据约束 | Codex |
...
| 2026-06-15 | SDK / Contracts 版本提升到 `0.4.2`，根应用版本保持 `0.4.13`，用于发布已有环境导入 workflow 场景契约、代理读取能力和环境列表绑定 IP 展示；SDK / Contracts PyPI 发布与本轮验证证据已补齐 | Codex |
| 2026-06-16 | 根应用版本提升到 `0.4.14`，用于发布已导入指纹浏览器环境的来源代理同步与 IP 表唯一匹配绑定能力；正式 tag / release、0.4.14 客户端包与真机升级证据仍需后续补齐 | Codex |
| 2026-06-16 | 根应用版本提升到 `0.4.15`，用于修复 VirtualBrowser 来源代理解析优先级，避免本地转发 URL 被保存为绑定 IP；正式 tag / release、0.4.15 客户端包与真机升级证据仍需后续补齐 | Codex |
| 2026-06-18 | 根应用版本提升到 `0.4.16`，用于修复来源代理同步匹配规则，按 `host + port` 唯一命中 IP 表；macOS 客户端包后续已于 2026-06-19 补齐，正式 tag / release 与 Windows 真机升级证据仍需后续补齐 | Codex |
| 2026-06-19 | 重新发布 macOS `0.4.16` 客户端升级包：删除远端旧 DMG 后重新生成并上传 `Crawler4j-0.4.16.dmg` 与 `appcast.xml`，公网下载 URL 已返回 `200` | Codex |
| 2026-06-20 | 根应用版本提升到 `0.4.17`，用于任务监控作业禁用状态、REM 固定运行模板安全门和来源代理同步匹配规则修复；正式 tag / release、0.4.17 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-22 | 根应用版本提升到 `0.4.18`，用于 VirtualBrowser 随机指纹创建期不下发具体指纹字段和 `chrome_version=139..145` 随机化；正式 tag / release、0.4.18 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-28 | 根应用版本提升到 `0.4.19`，用于 `browser.drag` 连续轨迹生成与框架自检 trace 能力；正式 tag / release、0.4.19 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-28 | 根应用版本提升到 `0.4.20`，用于 `browser.drag natural` 体感时长、约 60Hz 采样与固定 seed 默认混入运行随机盐的框架自检能力；正式 tag / release、0.4.20 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-29 | 根应用版本提升到 `0.4.21`，用于 VirtualBrowser 随机指纹代理出口 geo 校准、创建后轻量验收、风险环境标记与默认调度跳过；正式 tag / release、0.4.21 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-29 | 根应用版本提升到 `0.4.22`，用于 VirtualBrowser 随机指纹语言参数去重；正式 tag / release、0.4.22 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-06-30 | 根应用版本提升到 `0.4.23`，用于本轮 GitHub release 收口；正式 tag / release 将随本轮 PR 合并后创建 | Codex |
| 2026-07-05 | 根应用版本提升到 `0.4.24`，用于 REM 批量环境清理预览的模块候选 scope 修复；正式 tag / release、0.4.24 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
| 2026-07-08 | 根应用版本提升到 `0.4.26`，用于 VirtualBrowser 创建环境指纹自洽与稳定性优化；正式 tag / release、0.4.26 客户端包与 Windows 真机证据仍需后续补齐 | Codex |
