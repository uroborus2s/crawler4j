...
| 对象 | 当前值 | 说明 |
|---|---|---|
| 根应用包版本 | `0.4.34` | 当前仓库源码事实；承接 VirtualBrowser 启动后完整代理状态回写 |
| 根应用运行时版本 | `0.4.34` | 由运行时代码从包元数据或 `packages/crawler4j/pyproject.toml` 解析 |
...
- 当前根应用源码为 `0.4.34`，但 Git tag 在本次发布前仍停留在 `v0.2.0`；如果不显式分层，维护者会误以为当前源码版本已完成正式发布。
...
1. 确认 `packages/crawler4j/pyproject.toml`、运行时版本显示和 README 仍统一指向目标正式版本 `0.4.34`
...
6. 为根应用补打对应 `0.4.34` Git tag 与正式 release 资产
