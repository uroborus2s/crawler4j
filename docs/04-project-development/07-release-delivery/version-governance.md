...
| 对象 | 当前值 | 说明 |
|---|---|---|
| 根应用包版本 | `0.4.42` | 当前仓库源码事实；避免保活任务在并发已满时重复探测环境候选 |
| 根应用运行时版本 | `0.4.42` | 由运行时代码从包元数据或 `packages/crawler4j/pyproject.toml` 解析 |
...
- 当前根应用源码为 `0.4.42`，但 Git tag 在本次发布前仍停留在 `v0.2.0`；如果不显式分层，维护者会误以为当前源码版本已完成正式发布。
...
1. 确认 `packages/crawler4j/pyproject.toml`、运行时版本显示和 README 仍统一指向目标正式版本 `0.4.42`
...
6. 为根应用补打对应 `0.4.42` Git tag 与正式 release 资产，并在每个目标平台执行 `--crawler4j-verify-http-runtime`
...
| 日期 | 变更内容 | 变更人 |
|---|---|---|
| 2026-08-09 | 根应用 / 运行时源码提升到 `0.4.42`；保活调和仅在存在等待任务且有剩余并发时探测 `@env_candidates`，避免已满并发时每 5 秒重复调用模块候选函数 | Codex |
