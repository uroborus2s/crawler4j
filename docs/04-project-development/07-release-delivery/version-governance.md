...
| 对象 | 当前值 | 说明 |
|---|---|---|
| 根应用包版本 | `0.4.0` | 当前仓库 HEAD 已切到 0.4.0 破坏性升级目标版本 |
| 根应用运行时版本 | `0.4.0` | 由运行时代码从包元数据或 `packages/crawler4j/pyproject.toml` 解析 |
| 最近正式发布 tag | `v0.2.0` | 最新已知正式发布 |
| SDK | `0.4.0` | 当前工作区 SDK 版本；CLI 命令树、脚手架与开发者文档已同步到 0.4.0，build 证据已补，正式 publish 证据待补 |
| Contracts | `0.4.0` | 当前工作区 Contracts 版本；共享契约与 SDK / Core README、发布文档口径已同步收口，build 证据已补，正式 publish 证据待补 |
| docs-stratego 主文档版本 | 待正式发布前确认 | 当前源码文档入口已把 0.4.0 作为当前主线、0.3.x 作为历史维护；发布站点切换仍需随正式发布动作确认 |

## 3. 为什么这样定义

- 过去的问题不是“版本号多少”，而是同一份仓库里同时存在根包版本、运行时版本和 tag 口径漂移。
- 当前工作区已经切到 `0.4.0`，但 Git tag 仍停留在 `v0.2.0`；如果不显式分层，维护者会误以为 `0.4.0` 已正式打 tag 并发布。
...
1. 确认 `packages/crawler4j/pyproject.toml`、运行时版本显示和 README 仍统一指向目标正式版本 `0.4.0`
...
6. 为根应用补打对应 `0.4.0` Git tag 与正式 release 资产
