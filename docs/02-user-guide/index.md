...
第一次只拿这张图认入口，不拿图里的数字当验收标准。你要确认的是：左侧 6 个导航存在、仪表盘统计卡存在、底部日志区域存在。
...
- 在系统设置里统一配置代理、外部浏览器、日志和版本信息；日志设置会立即热更新，调度器心跳日志不会在 `INFO` 下持续刷屏。

## 新手先只记住这 3 个动作

如果你第一次接触 `crawler4j`，先不要记全部术语。先只按下面 3 个入口走：

1. `系统设置`：先把代理、外部浏览器和日志规则核对正确。
...
| `使用文档` | 在客户端里直接阅读内置文档中心，里面包含使用指南和开发指南；右侧直接展示正文，不再重复显示页面说明头部，左侧分组标题整行可点，文内大图会按阅读区宽度缩放，超链接会用高对比浅蓝并带下划线显示 |
| `系统设置` | 改全局设置：主题、代理、浏览器连接、日志、版本信息                                                                      |
