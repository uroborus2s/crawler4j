...
| `同步来源代理` | 重新读取已导入指纹浏览器环境的来源代理，唯一命中 IP 表时补齐绑定关系；未命中时只保存静态代理 |
| `▶ 运行` | 让环境进入可连接或活跃状态 |
| `⏸ 暂停` | 暂停环境 |
| 编辑 | 修改环境定义 |
| 销毁 | 删除环境 |
...
如果环境是在旧版本中导入的，列表里的 `绑定 IP` 可能为空，或误显示为 `127.0.0.1:本地端口` 这类指纹浏览器本地转发地址。此时点击 `同步来源代理`，客户端会从外部指纹浏览器回读该环境当前代理，优先使用来源返回的结构化 `host/port/protocol/user/pass` 作为真实代理源地址；只有唯一命中 IP 表条目时才写入绑定，未命中 IP 表时会跳过或清除本地错误绑定，不会把非 IP 表代理保存成绑定 IP。这个动作不会修改外部浏览器里的代理配置。
