...
| `环境类型` | 当前环境的类别 |
| `选择方式` | 选择已有环境时，默认是 `指定环境`；需要按账号状态、等级、黑名单等业务规则动态筛选时再切到 `候选函数` |
| `指定环境` | 保存为固定 `env_id`；下拉框只列出当前模块可用的就绪浏览器环境，包括未归属环境和已归属当前模块的环境 |
| `候选函数` / `候选参数` | 指定模块的环境候选函数，并按需给候选函数传一组参数；不需要参数时留空即可 |
...
- 项目要求固定环境时，优先用 `指定环境` 复用已有环境；如果目标环境没出现在下拉里，通常说明它不是就绪浏览器环境、正在被租约占用，或已经归属其他模块
- 项目需要按账号状态、黑号状态、会员等级等业务条件自动挑环境时，再使用 `候选函数`
...
| `已禁用` | 只有确认现场允许操作后再点 `启用`；启用只解除锁定，不会自动启动 |
...
| `禁用` | 锁定作业，防止误点执行、启动或调试；若作业仍有活跃实例，系统会请求停止 |
| `启用` | 解除作业禁用锁；作业会回到暂停态，需要你再手动执行或启动 |
| 编辑按钮 | 重新修改作业 |
| 删除按钮 | 删除作业定义 |
...
| `同步来源代理` | 重新读取已导入指纹浏览器环境的来源代理，按 `host + port` 唯一命中 IP 表时补齐绑定关系；未命中时只保存静态代理 |
| `↻ 重新检测指纹风险` | 对已标记为风险的指纹浏览器环境重新读取远端参数并更新本地风险状态 |
| `▶ 运行` | 让环境进入可连接或活跃状态 |
| `⏸ 暂停` | 暂停环境 |
| 编辑 | 修改环境定义 |
| 销毁 | 删除环境 |
...
0.4.0 不再在环境列表里展示资源池资格状态。模块是否会使用某个环境，由运行模板里的固定 `env_id` 或模块 `candidates/` 下的 `@env_candidates` 候选函数实时决定。运行模板的 `指定环境` 下拉框不会列出全量环境，只展示当前模块可用的就绪浏览器环境。

如果环境列表的 `风险` 列显示 `风险: ...`，表示该环境的指纹验收 metadata 当前未通过。调度默认跳过这类环境，手动运行入口也不会直接启动；系统不会自动删除它。点击行内 `↻` 后，客户端会重新读取当前远端环境参数并只更新 `fingerprint.validation` metadata：通过时状态改为 `passed`，失败时继续保留 `risk` 并更新原因和详情。这个动作不会修改代理、不会重新随机指纹、不会改 WebRTC 配置，也不会清空登录态。

新建 VirtualBrowser 环境时，语言、时区和定位跟随代理 IP，屏幕跟随运行浏览器的电脑，UA 使用浏览器默认模式。CPU/内存继续限定为原有常用组合（4/8、4/16、8/16、8/32、12/32），池外组合在随机创建后重新选择。Speech Voices 使用随机模式及空配置对象，不再下发固定语音列表。字体和图像/音频随机指纹保留浏览器生成的结果；IP 表中的地理信息也不再强制写入新环境。主动重新随机指纹使用同一策略，已有环境不会自动迁移。这些设置不代表目标网站的风控验证已经通过。
...
如果环境是在旧版本中导入的，列表里的 `绑定 IP` 可能为空，或误显示为 `127.0.0.1:本地端口` 这类指纹浏览器本地转发地址。此时点击 `同步来源代理`，客户端会从外部指纹浏览器回读该环境当前代理，优先使用来源返回的结构化 `host/port` 作为真实代理源端点；只要该 `host + port` 在 IP 表中唯一命中，就会写入绑定，不再比较协议、用户名或密码。未命中 IP 表时会跳过或清除本地错误绑定，不会把非 IP 表代理保存成绑定 IP。这个动作不会修改外部浏览器里的代理配置。
...
新建 IP 池和运行模板的默认绑定策略是 `最久未使用`：客户端会优先选择从未被绑定过的 IP，其次选择最近一次使用时间最早的 IP。`最少绑定数量` 仍保留为可选策略，适合现场明确需要按当前绑定数做负载均衡的场景。

每条 IP 都有人工状态：

- `可用`：允许后续环境绑定时从池里选中这条 IP
- `不可用`：后续环境绑定会跳过这条 IP，但不会自动解绑已经绑定到这条 IP 的现有环境

如果现场确认某条代理异常，可以在该条 IP 的 `操作` 栏点击 `停用`；修复或更换代理后，再点击 `启用` 让它重新进入后续绑定候选。这个状态由人工维护，和 `测试` 结果互不自动联动。
