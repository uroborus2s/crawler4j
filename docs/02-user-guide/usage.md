...
| --- | --- |
| `运行中` | 当前作业处于活动态 |
| `环境启动中` | 系统已经接单，正在创建环境并启动浏览器；启动条会在环境真正可用后自动消失 |
| `执行中` | 对手动批次任务来说，当前这轮正在跑；这时主按钮会切成 `⏹ 中止` |
| `中止中` | 你已经发出中止请求，系统正在执行 cleanup 并关闭或删除环境 |
| `已暂停` | 作业被暂停，不会继续启动新任务 |
| `已完成` | 当前批次已经结束 |
| `异常` | 作业进入错误态，需要看详情定位 |
...
| 你看到的状态 | 下一步动作 |
| --- | --- |
| `环境启动中` | 先别重复点按钮，等启动条消失；如果顶部立刻弹出错误提示，再去看作业详情和主日志 |
...
| `⏹ 中止` | 手动批次任务已经启动，但你需要手动停止并清理环境 |
| `启动` | 持续保活作业恢复运行 |
| `暂停` | 暂停作业，不再继续补任务或触发新批次 |
| 编辑按钮 | 重新修改作业 |
| 删除按钮 | 删除作业定义 |
...
- `保留环境中止`：关闭环境，但不删除
- `删除环境中止`：删除本次创建的环境；只有运行模板是“创建环境”模式时才会出现
...
| --- | --- |
| `➕ 创建环境` | 手动新建一个环境 |
| `从已有环境导入` | 从外部浏览器提供方拉取“来源里有、本地还没有同名记录”的环境，导入后直接启动并执行所选模块 workflow |
| `▶ 运行` | 让环境进入可连接或活跃状态 |
| `⏸ 暂停` | 暂停环境 |
| 编辑 | 修改环境定义 |
| 销毁 | 删除环境 |
...
如果现场使用固定环境池，列表里的 `可用状态` 会合并宿主 `env_metadata` 中的资源池资格卡片。`可用` 表示至少对应资源池可发号，`部分可用` 表示只对部分模块/资源池可用，`不可用` 会在悬停提示中展示模块、资源池和停发原因。
...
1. 点击工具栏里的 `从已有环境导入`
2. 依次选择 `环境来源`、`目标模块`、`模块工作流`
3. 在 `未同步环境列表` 里单选一个环境
4. 点击 `导入并执行`

如果你看到风险提示，表示当前 workflow 没有明确声明自己适配“已有环境导入”场景。这个提示不会拦住执行，只是提醒你自行判断该环境是否已经满足登录态、页面状态或前置步骤要求。
...
测试结果弹窗使用客户端公共深色弹窗组件，标题栏和内容区都是深色背景，便于在 macOS 等系统主题下稳定阅读出口 IP、代理地址和 HTTP 状态。
...
| 点击 `执行一次` 没反应 | `系统设置`、任务状态、运行模板 | 多半是全局设置或模板问题 |
| 作业有状态变化但业务失败 | 作业详情 `结果/错误`、任务日志 | 已进入执行链，继续看具体失败点 |
| 环境一直不可用 | `环境管理` | 优先看环境状态和外部浏览器连接 |
| 需要走代理但一直访问失败 | `系统设置 -> 网络`、`环境管理 -> IP 池管理` | 代理模式、IP 池或浏览器链路不对 |
