...
如果环境是在旧版本中导入的，列表里的 `绑定 IP` 可能为空，或误显示为 `127.0.0.1:本地端口` 这类指纹浏览器本地转发地址。此时点击 `同步来源代理`，客户端会从外部指纹浏览器回读该环境当前代理，优先使用来源返回的结构化 `host/port/protocol/user/pass` 作为真实代理源地址，生成预览后再同步到本地环境记录；这个动作不会修改外部浏览器里的代理配置。
