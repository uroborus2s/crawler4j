...
| `自动检查更新` | 是否允许客户端自动检查更新 | 若当前交付的是带 Sparkle 的 macOS 内部版，则会同步控制 Sparkle 自动检查；若当前交付的是通过 Velopack Setup 安装的 Windows 正式版，则会作为宿主更新偏好保存；其余交付形态下只保留为全局偏好开关 |
...
- 修改后会立即热更新，不需要重启应用
- 像 `APScheduler` 这种第三方调度器的高频心跳日志，会额外收口到至少 `WARNING`；当前 Service Job 的 5 秒补位巡检已不再走 `APScheduler` 周期 job 的“执行成功”日志，而是由 ATM 控制器在主事件循环里自行兜底调和
