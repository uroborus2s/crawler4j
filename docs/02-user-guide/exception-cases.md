...
3. 确认交付方给的是当前批次正式包。当前客户端不会兼容旧协议模块；如果包里仍使用 `core-native-v1`、`module.yaml.data` 或固定目录导出的 0.3.x 契约，安装后会直接不能启用，但这只是常见原因之一。
...
2. 确认这条作业真的是你要手动触发的那条作业，并且运行模板已经选好模块、流程和环境来源。
3. 回到 `模块管理`，确认目标模块状态仍是 `已启用`。
4. 如果任务已经进了 `执行中` 但明显卡住，可以先点 `⏹ 中止`；宿主会统一停止任务并回收环境。需要删除未认领或废弃环境时，后续到 `环境管理` 页面点击 `清理环境`。
5. 如果顶部错误提示里已经出现 `VirtualBrowser launchBrowser 失败`、`DevTools port` 或类似字样，优先按这个错误检查外部浏览器本体是否真的能打开、调试端口是否可用。
6. 多并发启动 VirtualBrowser 环境时，新版客户端会在宿主侧串行执行创建、打开和 Playwright 连接，避免外部浏览器管理 API 被同时请求拖慢；如果窗口仍出现长时间无法拖动或点击，先把目标并发降到 `1` 做对照。
7. 回到 `配置中心`，确认外部浏览器端口、程序位置和代理配置已经保存正确。
...
5. 如果多个指纹浏览器环境同时启动、回收、关闭或销毁，客户端会按顺序调用外部浏览器管理 API；短时间显示处理中属于正常等待，但如果窗口完全无法拖动或系统资源飙高，继续保留日志和任务时间点给研发排查。
6. 如果任务依赖代理或 IP 池，再确认池存在、池里有可用 IP，且模板真的绑定到了这个池。
