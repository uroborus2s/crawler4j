...
`打开应用 -> 核对配置中心 -> 安装模块 -> 配置运行模板 -> 新建作业 -> 执行一次 -> 打开作业详情看结果`
...
- 左侧有 `仪表盘`、`任务监控`、`环境管理`、`模块管理`、`使用文档`、`配置中心` 6 个入口。
...
如果你拿到的是 Windows 正式版，第一次请优先确认自己运行的是 Velopack 正式发布产物：要么通过 `Setup.exe` 安装后从开始菜单/桌面快捷方式启动，要么使用 Velopack 产出的 `Portable.zip` 解压包；直接打开裸 `PyInstaller onedir` 目录，后面的 `配置中心 -> 应用升级 -> 检查更新` 不会走宿主自更新。
...
如果模块自己的功能页、数据页或管理页已经集成到宿主，只有配置为菜单入口的页面才会出现在模块详情的左侧菜单里；详情页等二级页面通常由页面内操作打开。
...
也就是说，你是在“新建作业”弹窗里点 `配置运行模板` 进入，不是在 `配置中心` 或 `模块管理` 里找它。
...
## 还分不清“配置中心 / 模块配置 / Workflow 配置 / 运行模板”时，直接按这个例子记

| 你要改的内容 | 去哪里改 |
| --- | --- |
| HTTP 代理、浏览器端口、程序路径、ATM 收尾预算 | `配置中心` |
