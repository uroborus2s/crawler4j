...
如果你拿到的是 Windows 正式版，第一次请优先确认自己运行的是 Velopack 正式发布产物：要么通过 `Setup.exe` 安装后从开始菜单/桌面快捷方式启动，要么使用 Velopack 产出的 `Portable.zip` 解压包；直接打开裸 `PyInstaller onedir` 目录，后面的 `系统设置 -> 关于 -> 检查更新` 不会走宿主自更新。
