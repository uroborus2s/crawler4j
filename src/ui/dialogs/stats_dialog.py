...
class StatsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        stats_layout.addWidget(
            self._create_stat_card("已完成", str(completed), "#a6e3a1")
        )
        stats_layout.addWidget(
            self._create_stat_card("已废弃", str(discarded), "#f38ba8")
        )
        ...
        detail_group.setStyleSheet(
            "background-color: #313244; border-radius: 12px; padding: 16px;"
        )
        ...
        detail_layout.addWidget(
            self._create_progress_bar("通过率 (通过 / 完成)", success_rate, "#a6e3a1")
        )

        # Reject rate
        reject_rate = (rejected / completed * 100) if completed > 0 else 0
        detail_layout.addWidget(
            self._create_progress_bar("拒绝率 (拒绝 / 完成)", reject_rate, "#f38ba8")
        )
        ...
        detail_layout.addWidget(
            self._create_progress_bar(
                "完成率 (完成 / 总领题)", completion_rate, "#89b4fa"
            )
        )
    ...
    def _create_progress_bar(
        self, label_text: str, percentage: float, color: str
    ) -> QWidget:
