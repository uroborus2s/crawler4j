...
class DashboardPage(QWidget):
    ...
    def _load_data(self):
        ...
        for row, env in enumerate(envs):
            ...
            if ctrip:
                phone_num = ctrip.get("phone_number", "")
                cc = ctrip.get("country_code", "+86")
                ctrip_phone = f"{cc} {phone_num}"
                if len(phone_num) >= 7:
                    ctrip_phone = f"{cc} {phone_num[:3]}****{phone_num[-4:]}"
            ...
            self.env_table.setItem(
                row, 2, QTableWidgetItem(labor["phone"] if labor else "-")
            )
    ...
    def _on_env_status_changed(self, env_id: int, status: str):
        """Slot for environment status changes."""
        # Full reload to maintain sync with Repository
        self._load_data()
