...
class SMSReceiver:
    ...
    def __init__(
        self, base_url: str, api_key: str, platform_type: str = "", phone: str = ""
    ):
    ...
    async def get_code(self, timeout: int = 120, interval: int = 5) -> Optional[str]:
        ...
        """
        logger.info(
            f"开始获取验证码: {self.phone} (平台: {self.platform_type}, 超时: {timeout}s)"
        )
        ...
        while time.time() - start_time < timeout:
            try:
                if self.platform_type == "haoma":
                    ...
                    # Usually: GET /api/handler?action=getSms&token=TOKEN...
                    # Note: This is a heuristic implementation.
                    # Ideally we need the 'item_id' (project ID) as well,
                    # but here we assume it's pre-configured or part of config.
                    # For now, we assume the base_url includes the endpoint.
    ...
    @classmethod
    async def fetch(
        cls, phone: str, url: str, key: str, p_type: str = "", timeout: int = 60
    ) -> Optional[str]:
