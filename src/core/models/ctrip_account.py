...
class CtripAccount:
    ...
    registered_at: datetime | None = None  # 账号注册时间
    ...
    def from_dict(cls, data: dict) -> "CtripAccount":
        ...
        sms_verify_type = (
            data.get("sms_verify_type") or data.get("sms_platform_type") or "manual"
        )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            if isinstance(self.account_type, AccountType)
            else self.account_type,
            "sms_verify_type": self.sms_verify_type.value
            if isinstance(self.sms_verify_type, SmsVerifyType)
            else self.sms_verify_type,
