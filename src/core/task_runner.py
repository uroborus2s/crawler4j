...
class TaskRunner:
    ...
    async def run(self) -> TaskResult:
        ...
        if not env_id:
            return TaskResult(result_type=TaskResultType.ERROR, message="环境 ID 无效")
        ...
        try:
            ...
            return TaskResult(result_type=TaskResultType.ERROR, message=str(e))
        finally:
            # 5. 更新环境状态
            if env_id:
    ...
    def _convert_result(self, workflow_result: WorkflowResult) -> TaskResult:
        ...
        type_mapping = {
            ...
            WorkflowResultType.NETWORK_ERROR: TaskResultType.ERROR,  # 网络错误
            WorkflowResultType.ERROR: TaskResultType.ERROR,
        }

        return TaskResult(
            result_type=type_mapping.get(
                workflow_result.result_type, TaskResultType.ERROR
            ),
