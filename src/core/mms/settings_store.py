...
from src.core.persistence.kv_store import get_kv_store
...
class ModuleSettingsStore:
    ...
    对历史 `module:{name}:config` KV 数据保留只读兼容。
    """
    ...
    def read_settings(self, scope: str, key: str) -> Any:
        ...
        if scope == self.MODULE_SCOPE:
            legacy = get_kv_store().get(f"module:{key}:config")
    ...
    def delete_settings(self, scope: str, key: str) -> bool:
        ...
        if scope == self.MODULE_SCOPE:
            deleted = get_kv_store().delete(f"module:{key}:config") or deleted
