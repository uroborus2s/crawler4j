...
def _write_debuggable_ctrip_module(base_dir: Path) -> tuple[Path, Path, int, Path, int]:
    module_dir = base_dir / "ctrip_crawler"
    tasks_dir = module_dir / "tasks"
    tasks_dir.mkdir(parents=True, exist_ok=True)

    init_source = dedent(
        """
        from .tasks.ctrip_login_task import run_login


        async def init_env(context):
            accounts = context.get_config("ctrip_accounts", [])
            if accounts:
                context.state["ctrip_account_phone"] = accounts[0]["phone_number"]
        ...
        async def run_login(context):
            ...
            if context.page:
                await context.page.goto(
                    context.get_config("ctrip_login_url", "about:blank"),
    ...
    (tasks_dir / "ctrip_login_task.py").write_text(task_source, encoding="utf-8")

    breakpoint_file = tasks_dir / "ctrip_login_task.py"
...
async def test_task_debug_session_hits_model_breakpoint(tmp_path, monkeypatch):
    ...
    module_dir, breakpoint_file, breakpoint_line, module_frame, module_line = _write_debuggable_ctrip_module(tmp_path)
