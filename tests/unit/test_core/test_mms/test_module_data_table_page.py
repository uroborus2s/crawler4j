...
def test_module_data_table_page_declares_schema_and_renders_display_fields(qtbot, monkeypatch):
    ...
    page = ModuleDataTablePage("ctrip", "accounts")
...
def test_module_data_table_page_routes_add_and_edit_to_module_handlers(qtbot, monkeypatch):
    ...
    def fake_handler(self, handler_name, *args):
        ...
        rows = kv.get(_dataset_key("ctrip", "accounts")) or []
        if handler_name == "create_account_from_ui":
            ...
            kv.set(_dataset_key("ctrip", "accounts"), rows)
    ...
    assert kv.get(_dataset_key("ctrip", "accounts"))[0]["account_status"] == "blocked"


def test_module_data_table_page_refresh_overwrites_legacy_schema(qtbot, monkeypatch):
    kv = get_kv_store()
    kv.set(
        _meta_key("ctrip", "accounts"),
        {
    ...
    schema = kv.get(_meta_key("ctrip", "accounts"))
...
def test_module_data_table_page_builds_devlink_context_with_settings(qtbot, monkeypatch):
    ...
    assert context.config["module_name"] == "ctrip"
