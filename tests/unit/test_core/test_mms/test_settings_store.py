...
def test_settings_store_reads_legacy_module_config_from_kv_store(temp_data_dir):
    from src.core.mms.settings_store import ModuleSettingsStore
    from src.core.persistence import get_kv_store

    get_kv_store().set("module:demo_module:config", {"legacy": True, "timeout": 30})

    store = ModuleSettingsStore()

    assert store.read_module_settings("demo_module") == {"legacy": True, "timeout": 30}
