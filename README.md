...
- `scripts/deploy_windows_release.py`：`uv run deploy-windows-release` 背后的 Windows 发布+上传入口；复用 `package-windows-release` 生成 Velopack 产物，再用 `rsync` 上传到 `CRAWLER4J_UPDATE_UPLOAD_TARGET/win/`
