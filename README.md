...
当前版本线不等于已经完成正式 tag / GitHub release。`crawler4j 0.4.13` 推进桌面宿主与 Core 运行时包版本，用于承接已有环境导入 workflow 契约、`env.get_proxy` 当前代理读取、多环境导入批次元数据，以及在环境管理列表展示环境绑定的代理 IP；`crawler4j-sdk 0.4.2`、`crawler4j-contracts 0.4.2` 用于对外发布这批 SDK/Contracts 契约。当前最新已记录 macOS Sparkle 客户端升级包仍为 2026-05-26 生成并上传的 `0.4.3`；`0.4.13` 客户端包、`ctrip` 真实站点 E2E、Windows 真机安装/升级证据、Git tag / GitHub release 与正式交付批次仍需继续收口。
