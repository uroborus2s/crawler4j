...
- `REQ-012`: Hosted UI DataTable must support current-page multi-select bulk update | Priority: P1 | Status: HUMAN_APPROVED_READY_FOR_COMMIT_WITH_CONCERN
  Constraint: top-level `selection_mode` accepts `none/single/multi` and defaults to `single`; Core passes only ordered, type-sensitive unique `primary_keys` and form `payload`; module handlers own business validation and `managed_dataset` updates; omitted fields keep legacy single-selection behavior. Task 1 / 2 reviews and Task 3 target gates passed; full unit has two unrelated version-README drift failures, so overall review and human confirmation remain separate gates.
