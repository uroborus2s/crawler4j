...
- `TC-024`: 模块快照数据与审计事件已拆成两条正式能力面：`module_audit_events` / `db.append_event` / `db.query_events` 负责 append-only 历史，`module_datasets` 继续承载快照型 records，且当前已改为“一条 record 一行”并覆盖 legacy `records_json` 自动迁移；覆盖见 `test_module_data_store.py`, `test_runtime_capabilities.py`, `test_data_capability.py`.
