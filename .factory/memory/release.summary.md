...
- Current app package version: `0.4.12`
- Runtime package version: `0.4.12`
- SDK version: `0.4.1`
- Contracts version: `0.4.1`

Evidence status:

- Root build: latest recorded artifact passed on 2026-05-26 for `crawler4j 0.4.3`; current `crawler4j 0.4.12` package build is not yet refreshed
- Desktop PyInstaller / macOS Sparkle bundle: latest recorded artifact passed on 2026-05-26 for `crawler4j 0.4.3`; current `crawler4j 0.4.12` desktop bundle is not yet refreshed
...
- Full test/lint gate: historical full gate passed on 2026-05-18 for the 0.4.2 root version bump; current 0.4.12 version bump, Hosted UI row action params dispatch, and task pause bound run_status release scope passed targeted regression `67 passed`, target `ruff check`, `uv lock --check`, `.factory/project.json` JSON validation, and `git diff --check`
...
- Local code quality gate: PASS for the 0.4.12 version bump and targeted Hosted UI row action / pause release regression scope
