...
- `API-008`: 模块 UI V1 目标契约已定稿但尚未实现：模块不再导出 `ui:*` / `PyQt6` 页面，而是通过 `ui_extension.pages[]`、`ui.declare_page` 和 `ui.declare_data_table` 声明宿主管理页；第一版公开控件固定为 `Page`、`Section`、`Text`、`Button`、`DataTable`。
