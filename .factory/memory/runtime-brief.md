...
- 当前 0.4.x Hosted UI 批量导入方案：已登记 `REQ-010` / `API-019` / `CR-016` 和 `TASK-030` ~ `TASK-034`，当前为方案和任务拆分阶段。目标是让页面 / `DataTable` toolbar 声明导入按钮，宿主读取 `.xlsx/.csv` 文件或剪贴板文本并解析为 import payload，模块只接收结构化行数据；宿主需限制文件类型、大小和最大行数，并对 token/cookie/password 等敏感字段日志脱敏。代码实现与 `TC-060` 验证尚未启动。
- 最近交接包：无
- 最近快照：无
