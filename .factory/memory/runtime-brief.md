...
- 当前版本：根应用 / runtime `0.4.30`；SDK `0.4.4`；Contracts `0.4.3`
- 当前正式 Git tag：`v0.2.0`
...
- Contracts / SDK PyPI 发布和隔离安装通过；客户端源码版本已到 `0.4.30`，不等于桌面安装包已发布。

## 开放风险

- `ctrip` 真实站点 DevLink / ZIP E2E 仍未闭环。
- Windows 签名、安装、自更新真机证据仍缺。
- 0.4.30 客户端桌面包、Git tag / GitHub release 资产和正式交付批次仍需单独收口。
