...
- 当前 0.4.x REM 批量环境清理预览：宿主点击“清理环境”会合并孤岛、未认领、owner 缺失和模块 `@env_cleanup_candidates` 候选；DevLink 模块在枚举 cleanup 入口前会用开发态 `TaskContext` 刷新 runtime descriptor，避免旧 descriptor cache 漏掉新 `cleanups/` 文件。模块候选仍必须满足同模块 claimed claim、`env_binding_field` 业务绑定、环境 `READY/PAUSED`、无租约、无关联任务、无活跃 task、未被 ACTIVE 固定运行模板引用。
