...
- 当前 0.4.x VirtualBrowser 随机指纹：`__randomize_fingerprint__` 创建期仍把 `chrome_version` 每次随机为 `139..145`，并下发最小自洽默认画像：`zh-CN` / `Asia/Shanghai`、常见屏幕 `mode=1`、常见 CPU/内存组合和 fonts/canvas/WebGL image/audio/client-rects/speech voices 随机模式；Core 继续剥离手工 `ua`、`ua-full-version`、`sec-ch-ua`、设备名和 MAC，避免身份字段与真实内核或外部浏览器生成结果冲突。运行模板 UI 默认把随机指纹视为托管模式，只展示浏览器版本、随机指纹开关和 IP 策略；取消随机化后才显示高级指纹参数。
