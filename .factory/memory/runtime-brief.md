...
- 阻塞项：0
- 开放风险：1
- 当前源码版本基线：`crawler4j 0.4.24`、`crawler4j-sdk 0.4.2`、`crawler4j-contracts 0.4.2`；最近正式 Git tag：`v0.2.0`
- 当前 PyPI 发布状态：`crawler4j-contracts 0.4.0` 重发曾被 PyPI 拒绝，因为对应 wheel 文件名曾上传后删除且不可复用；SDK / Contracts 已提升到 `0.4.2`，并已按 `contracts -> sdk` 顺序发布到 PyPI；根应用提升到 `0.4.24`，用于承接 REM 批量环境清理预览的模块候选 scope 修复。
...
- 当前 0.4.x VirtualBrowser 随机指纹：`__randomize_fingerprint__` 创建期仍把 `chrome_version` 每次随机为 `139..145`，并下发最小自洽默认画像：默认 `zh-CN` / `Asia/Shanghai`、现代加权屏幕池 `1920x1080` / `1536x864` 为主且保留少量 `2560x1440` / `1920x1200` / `1440x900` / `1680x1050` / `1366x768`、常见 CPU/内存组合和 fonts/canvas/WebGL image/audio/client-rects/speech voices 随机模式；若创建参数带代理，Core 会复用代理探测链路尽力获取真实出口 IP 的国家、城市、ASN 与时区，并用出口国家/时区覆盖语言与时区字段，探测失败则回退默认画像。创建后会调用 `getBrowserFullParameters(id)` 做轻量验收，warning 会写入 `fingerprint.validation.status=risk`，不自动删除环境。风险环境状态写入 `fingerprint.validation` metadata，环境列表展示风险原因并提供手动重新检测；复检只更新 validation metadata，不修改代理、指纹或 WebRTC 配置。Core 继续剥离手工 `ua`、`ua-full-version`、`sec-ch-ua`、设备名和 MAC，避免身份字段与真实内核或外部浏览器生成结果冲突。运行模板 UI 默认把随机指纹视为托管模式，只展示浏览器版本、随机指纹开关和 IP 策略；取消随机化后才显示高级指纹参数。
