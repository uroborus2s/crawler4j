...
- 当前 PyPI 发布状态：`crawler4j-contracts 0.4.0` 重发被 PyPI 拒绝，因为对应 wheel 文件名曾上传后删除且不可复用；`crawler4j-sdk 0.4.0` 暂停发布，避免其 `crawler4j-contracts>=0.4.0,<0.5.0` 依赖不可解析。后续外部发布需改发新版本号或先处理 PyPI 历史删除阻塞。
