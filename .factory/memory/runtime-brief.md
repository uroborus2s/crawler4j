...
- 阻塞项：0
- 开放风险：1
- 当前源码版本基线：`crawler4j 0.4.26`、`crawler4j-sdk 0.4.2`、`crawler4j-contracts 0.4.2`；最近正式 Git tag：`v0.2.0`
- 当前 PyPI 发布状态：`crawler4j-contracts 0.4.0` 重发曾被 PyPI 拒绝，因为对应 wheel 文件名曾上传后删除且不可复用；SDK / Contracts 已提升到 `0.4.2`，并已按 `contracts -> sdk` 顺序发布到 PyPI；根应用提升到 `0.4.26`，用于承接 VirtualBrowser 创建环境指纹自洽与稳定性优化。
...
- 当前 0.4.x ATM 运行模板环境选择：选择已有环境默认使用 `指定环境` 并写入固定 `env_id`；固定环境下拉只展示当前模块可用的 `READY + BROWSER + 无租约` 环境，且环境未归属或 `host.env_claim.owner_module` 等于当前模块，并会跳过 `fingerprint.validation.status=risk` 的风险环境。`@env_candidates` 仍用于账号状态、黑名单、等级等业务条件动态筛选，以及 Service Job 的候选等待队列；候选调度和容量计算默认也跳过风险环境。
- 当前 0.4.x REM 批量环境清理预览：宿主点击“清理环境”会合并孤岛、未认领、owner 缺失和模块 `@env_cleanup_candidates` 候选；DevLink 模块在枚举 cleanup 入口前会用开发态 `TaskContext` 刷新 runtime descriptor，避免旧 descriptor cache 漏掉新 `cleanups/` 文件。模块返回 `EnvCandidates` 时，宿主会把当前仍存在的环境 id 作为内部 scope 注入查询运行时，使业务表查询先限定到现存环境，再执行模块声明的排序和 limit，避免已删除环境占掉模块分页名额。模块候选仍必须满足同模块 claimed claim、`env_binding_field` 业务绑定、环境 `READY/PAUSED`、无租约、无关联任务、无活跃 task、未被 ACTIVE 固定运行模板引用；已存在但未通过 owner/claim/绑定门的模块候选会在预览中显示跳过原因。
- 当前 0.4.x VirtualBrowser 随机指纹：`__randomize_fingerprint__` 创建期仍把 `chrome_version` 每次随机为 `139..145`，并按宿主系统生成一次性稳定默认画像：Windows 使用 `Win64; x64`，macOS 使用 Macintosh，Linux 使用 `Linux x86_64`，同时下发默认 `zh-CN` / `Asia/Shanghai`、现代加权屏幕池、常见 CPU/内存组合、字体、语音、设备名、MAC 和 Canvas/WebGL image/audio/client-rects 扰动实际值；UI 传入的裸 `mode=1` 占位不会覆盖这些完整值。若创建参数带代理，Core 会复用代理探测链路尽力获取真实出口 IP 的国家、城市、ASN 与时区，并用出口国家/时区覆盖语言与时区字段，探测失败则回退默认画像。创建后会调用 `getBrowserFullParameters(id)` 做轻量验收，warning 会写入 `fingerprint.validation.status=risk`，不自动删除环境；验收会标记 `WOW64`、`location=0,0`、异常 CPU/内存组合、非本地转发的 `proxy.host` / `proxy.url` 不一致，以及字体/语音/扰动字段缺实际值。风险环境状态写入 `fingerprint.validation` metadata，环境列表展示风险原因并提供手动重新检测；复检只更新 validation metadata，不修改代理、指纹或 WebRTC 配置。运行模板 UI 默认把随机指纹视为托管模式，只展示浏览器版本、随机指纹开关和 IP 策略；取消随机化后才显示高级指纹参数。
- 当前 0.4.x ATM 作业状态：任务监控支持 `已禁用` 状态；禁用作业不能执行一次、启动或进入调试，禁用操作会请求停止已有活跃实例，启用只解除禁用锁、清理 job 停止标记并回到暂停态，不会自动启动。
