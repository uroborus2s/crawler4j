...
- Hosted UI field change and Form reset keep the same ownership split: Contracts/SDK own schema and handler diagnostics, renderer owns Qt state and responsive 1–3 column Form layout, a Core registry owns short-lived scoped handles/revisions, and the runtime action surface exposes only generic `ui.form.reset`. Modules decide whether and how to reset; no UI object or business effect crosses the boundary.
