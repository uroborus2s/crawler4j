...
- SDK / Contracts `1.1.1` 发布准备：已将 `crawler4j-sdk`、`crawler4j-contracts` 与脚手架默认 `sdk_version_range` 升级到 `1.1.1` 口径，并在开发/发布文档中固化 `on_cleanup` 为终态清理 hook 的正式规则；注意该版本号按业务要求定为 patch，但包含公开 API 删除风险 | 状态：DONE | 关联：`packages/crawler4j-sdk/pyproject.toml`、`packages/crawler4j-contracts/pyproject.toml`、`packages/crawler4j-sdk/src/cli/templates.py`
