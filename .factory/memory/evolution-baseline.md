...
- Keep VirtualBrowser random fingerprint as a managed creation mode: generate a small coherent default profile for language/timezone/screen/hardware/random-noise fields, but continue stripping hand-authored UA, Sec-CH-UA, device name and MAC so identity fields do not drift away from the real browser kernel.
- Keep VirtualBrowser random fingerprint geo as a creation-time probe, not a fixed UI burden: when a proxy is present, derive language/timezone from the real proxy exit country/timezone and validate saved parameters after `addBrowser`; failed probes should fall back to the default coherent profile instead of blocking creation or auto-deleting environments.
