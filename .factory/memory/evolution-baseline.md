...
- Keep VirtualBrowser lifecycle operations serialized at the provider boundary: `addBrowser`, `launchBrowser` and Playwright/CDP `connect` should not run concurrently for the same provider instance, because the external management API and CDP warm-up path can otherwise stall the shared desktop qasync loop during multi-env startup
