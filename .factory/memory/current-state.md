...
- 文档：根 `docs/index.md` 已收敛为四大模块目录树；第一部分 `docs/01-getting-started/` 已补齐项目概览与快速开始；第二部分 `docs/02-user-guide/` 已补齐安装/配置/使用说明；第三部分 `docs/03-developer-guide/` 已直接作为 module 开发指南，且各级概览页标题已统一为中文，并已把“模块只能使用 Core 注入的 `ctx.db` 最小数据能力”融入概念约束、TaskScript、CLI/UI、Core 能力与排错章节；Core 维护入口位于 `docs/04-project-development/08-operations-maintenance/core-maintainer-guide.md`
