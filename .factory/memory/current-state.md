...
- 最新修复：2026-04-21 已把 `data.db.module_datasets` 从旧 `(module_name, dataset_name, records_json)` 聚合快照表切换为按 `(module_name, dataset_name, record_index)` 一条 record 一行持久化；`db.list_records` / `db.replace_records` 与 `core:data_table` 的上层契约保持不变，`init_database()` 会在启动时自动把旧库聚合 JSON 拆行迁移；对应回归覆盖 `test_module_data_store.py`、`test_runtime_capabilities.py`、`test_module_data_table_page.py`、`test_settings_store.py`。
