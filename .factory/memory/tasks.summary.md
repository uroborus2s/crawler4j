...
- `CR-025` / `TASK-045`：Core/SDK/Contracts 的同步/异步环境候选、结构化 context、单次计算注入、候选 HTTP surface、scanner/manifest 与文档已实现；客户端/Contracts/SDK 源码版本为 `0.4.41` / `0.4.5` / `0.4.6`，三包本地构建、完整验证与客户端增量复评 `99/100` 通过；提交 `f3d2aceb` 与状态提交已推送 `origin/0.4.0`，不发布。
