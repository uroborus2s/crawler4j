...
Hosted UI 用户按钮、CRUD handler 和表单提交使用 `@ui_action`；浏览器页面自动化使用 `@page_action`，并由 workflow/component 通过 `ctx.run_page_action(...)` 调用。`@page_action` 不是内部拆分单元，不能在另一个 `@page_action` 中嵌套调用。
