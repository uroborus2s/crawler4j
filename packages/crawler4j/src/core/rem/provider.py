...
class PlaywrightProvider(BaseProvider):
    ...
    async def connect(self, env: Environment) -> bool:
        ...
        try:
            handle._context = (
                handle.browser.contexts[0] if handle.browser.contexts else await handle.browser.new_context()
            )
            handle._page = (
                handle.context.pages[0] if handle.context and handle.context.pages else await handle.context.new_page()
            )
...
class BitBrowserProvider(BaseProvider):
    ...
    async def create(self, config: dict[str, Any] | None = None) -> Environment:
        ...
        if (proxy_mode == ProxyMode.STATIC or proxy_mode == ProxyMode.POOL) and (
            raw_val := proxy_data.get("static_value")
        ):
        ...
        async with self._get_lifecycle_lock():
            browser_id = await client.create_browser(name, proxy=bit_proxy, group_id=group_id, fingerprint=fingerprint)
...
class VirtualBrowserClient:
    ...
    async def launch_browser(self, browser_id: int) -> str:
        ...
        for attempt in range(2):
            ...
            launch_error = str(data.get("error") or data.get("msg") or resp.text or f"HTTP {resp.status_code}")
            ...
            logger.error(
                f"[VirtualBrowser] launchBrowser failed: id={browser_id} status={resp.status_code} error={launch_error}"
            )
...
class VirtualBrowserProvider(BaseProvider):
    ...
    async def _list_existing_envs_unlocked(self) -> list[ProviderEnvInfo]:
        ...
        running_ids = {
            str(item.get("id")) for item in running_rows if isinstance(item, dict) and item.get("id") is not None
        }
