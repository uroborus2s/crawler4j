...
def test_cli_module_scaffold_flow_end_to_end(tmp_path: Path):
    ...
    assert manifest["sdk_version_range"] == ">=1.1.0"
...
def test_cli_rejects_removed_init_command_end_to_end(tmp_path: Path):
    result = _run_cli("init", "removed_command_project", cwd=tmp_path)
