...
async def test_drag_reports_physical_event_trace_for_self_check(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert trace["strategy"] == "continuous"
    assert trace["phase_names"] == ["continuous"]
    assert trace["sample_count"] == sum(len(phase["samples"]) for phase in trace["phases"])
    assert trace["sample_count"] >= sum(trace["steps"])
    assert all("dt" in sample and sample["dt"] > 0 for phase in trace["phases"] for sample in phase["samples"])
...
async def test_drag_modes_expose_distinct_phase_shapes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert natural["trace"]["strategy"] == "continuous"
    assert natural["trace"]["phase_names"] == ["continuous"]
...
@pytest.mark.asyncio
async def test_natural_drag_randomizes_continuous_profiles_and_vertical_motion(
    ...
    for seed in range(1, 24):
        ...
        assert trace["phase_names"] == ["continuous"]
        profiles.add(trace["continuous_profile"])
        saw_without_overshoot = saw_without_overshoot or trace["overshoot"] is None
        saw_without_settle = saw_without_settle or trace["settle"] is None
        saw_vertical_motion = saw_vertical_motion or (max(y_values) - min(y_values) > 0.5)

    assert len(profiles) >= 3
    assert saw_without_overshoot is True
    assert saw_without_settle is True
    assert saw_vertical_motion is True


@pytest.mark.asyncio
async def test_natural_drag_uses_continuous_samples_with_timing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("src.core.atm.browser_tools.asyncio.sleep", CapturedSleep(delays=[]))

    page = FakePage({".slider": FakeLocator(bbox={"x": 100.0, "y": 180.0, "width": 40.0, "height": 40.0})})
    tools = CoreBrowserTools(seed=29)
    tools.bind_task_context(TaskContext(env_id=1, task_name="demo_module", page=page, tools=None))

    trace = (await tools.drag(selector=".slider", delta_x=180.0, mode="natural", steps=24))["trace"]
    samples = [sample for phase in trace["phases"] for sample in phase["samples"]]

    assert trace["strategy"] == "continuous"
    assert trace["phase_names"] == ["continuous"]
    assert len(samples) >= 24
    assert all(sample["dt"] > 0 for sample in samples)
