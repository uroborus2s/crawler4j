...
@pytest.mark.asyncio
async def test_dispatcher_save_task_update_publishes_environment_starting_progress(monkeypatch):
    published = []
    dispatcher = TaskDispatcher.__new__(TaskDispatcher)
    dispatcher.repo = SimpleNamespace(save_task=AsyncMock())
    ...
    dispatcher.repo.save_task.assert_awaited_once_with(task)
    assert len(published) == 1
    assert published[0].type == EventType.TASK_PROGRESS
    assert published[0].data["phase"] == "environment_starting"
    assert published[0].data["task_id"] == "task-pending"
    assert published[0].data["job_id"] == "job-1"
