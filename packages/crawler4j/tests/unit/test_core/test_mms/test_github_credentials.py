...
def test_github_credential_store_roundtrip_encrypts_token(temp_data_dir: Path):
    ...
    raw_setting = get_config_center().get_internal("mms.github", "repo_token.example__private-repo")
    assert raw_setting is not None
    assert "ghp_secret_token_1234" not in str(raw_setting)
...
def test_github_credential_store_rejects_corrupted_payload():
    store = get_github_credential_store()
    get_config_center().set_internal(
        "mms.github",
        "repo_token.example__private-repo",
