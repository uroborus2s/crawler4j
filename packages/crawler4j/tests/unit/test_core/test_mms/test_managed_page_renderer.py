...
def test_managed_page_renderer_uses_query_pagination_when_handler_omits_page_fields(qtbot, tmp_path):
    module_name = "hosted_page_query_pagination_module"
    module_dir = write_module_tree(
        tmp_path,
        module_name,
        files={
            ...
            def query_stats_table(context: TaskContext, table_id: str, query, params=None):
                del context, table_id, params
                page = int(query.get("page", 1))
                page_size = int(query.get("page_size", 20))
                return {
                    "rows": [{"metric": "query_page", "value": f"{page}/{page_size}"}],
                    "total": 12,
                }
            """,
        },
    ...
    try:
        ...
        table.set_query({"page": 2, "page_size": 5})
        table.request_refresh()

        assert table.item(0, 1).text() == "2/5"
        assert table.page_label.text() == "2 / 3"
    finally:
        restore_module(service, original_registry, module_name)
